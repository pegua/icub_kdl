...
bool idynChain2kdlChain(iCub::iDyn::iDynChain & idynChain,KDL::Chain & kdlChain,std::vector<std::string> link_names,std::vector<std::string> joint_names,int max_links)
{
    int n_links, i;
    bool use_names;
    n_links = idynChain.getN();
    //In iDyn, the joints are only 1 DOF (not 0, not more)
    n_joints = idynChain.getN();
    ...
    for(i=0;i<n_links && i < max_links;i++) 
    {
        ...
        //DEBUG
        //cout << i;
        //printMatrix("inertia",idynChain.getInertia(i));
...
bool idynSensorChain2kdlChain(iCub::iDyn::iDynChain & idynChain,iCub::iDyn::iDynInvSensor & idynSensor ,KDL::Chain & kdlChain, std::vector<std::string> link_names,std::vector<std::string> joint_names, int max_links)
{
    bool use_names;
    ...
    for(i=0;i<n_links;i++) 
    {
        ...
        //( i == segment_link )
            ...
            yarp::sig::Matrix  H_sensor_link = (link_current.getH(0.0));
            link_current.setAng(angSensorLink);
            
            yarp::sig::Matrix H_0 = H_sensor_link  * (idynSensor.getH());
            yarp::sig::Matrix H_1 = localSE3inv(idynSensor.getH());
            //printMatrix("H_0",H_0);
            //printMatrix("H_1",H_1);
...
{
    std::cout << s << std::endl;
    for(int p=0;p < kdlChain.getNrOfSegments();p++)
    {
